...
class AllSearchNutritionPlanRecyclerAdapter(var plans: ArrayList<AllPlanNutritionSearchModel>) : ListAdapter<AllPlanNutritionSearchModel, AllSearchNutritionPlanRecyclerAdapter.ViewHolder>(
    ExerciseDiffUtil()
) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val plan_search: AllPlanNutritionSearchModel = getItem(position)
        ...
        fireBaseFirestore.collection("Planos de alimentação").document("Banco de Id")
            .collection(plan_search.planId).document("Gostos").get().addOnSuccessListener { documents ->
                if (documents != null) {
                    var foodNumberLikes = documents["Gostos"].toString()
                    holder.likeExerciseText.text = foodNumberLikes
    ...
    class ExerciseDiffUtil : DiffUtil.ItemCallback<AllPlanNutritionSearchModel>(){
        override fun areItemsTheSame(oldItem: AllPlanNutritionSearchModel, newItem: AllPlanNutritionSearchModel): Boolean {
            return oldItem.planId === newItem.planId
        }

        override fun areContentsTheSame(oldItem: AllPlanNutritionSearchModel, newItem: AllPlanNutritionSearchModel): Boolean {
